import { Component, input, output, computed, inject, signal, HostListener, ElementRef } from '@angular/core';
...
export class TaskList {
  ...
  // Track which task is expanded (only one at a time)
  expandedTaskId = signal<string | null>(null);

  // Close expanded task when clicking outside
  @HostListener('document:click', ['$event'])
  onDocumentClick(event: MouseEvent) {
    if (this.expandedTaskId() && !this.elementRef.nativeElement.contains(event.target)) {
  ...
  onExpandToggle(taskId: string) {
    // Toggle: if same task, collapse; if different, expand new one
